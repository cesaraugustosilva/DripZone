...
export async function getApiStatus() {
  if (apiStatusRequest) return apiStatusRequest;
  apiStatusRequest = request("/status")
    .then((data) => ({ state: "connected", label: "API conectada", data }))
    .catch((error) => ({ state: "disconnected", label: "API desconectada", error }))
    .finally(() => {
      window.setTimeout(() => {
        apiStatusRequest = null;
      }, 1000);
    });
...
export const getActivities = (params = {}) => {
  const query = new URLSearchParams(params);
  return request(`/activities${query.size ? `?${query}` : ""}`);
};
